...
const navItems = [
  { path: '/platform/dashboard', label: 'Dashboard', icon: DashboardIcon },
  { path: '/platform/tenants', label: 'Tenants', icon: TenantsIcon },
  { path: '/platform/users', label: 'Platform Users', icon: UsersIcon },
  { divider: true },
  { path: '/platform/audit', label: 'Global Audit Logs', icon: AuditIcon },
  { divider: true },
...
export default function PlatformLayout() {
  ...
  const { logout, platformRoles } = useAuth()
    ...
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', bgcolor: 'background.paper' }}>
      ...
      <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 1 }}>
        {platformRoles.map((role) => (
          <Chip
            key={role}
            icon={<GlobalIcon sx={{ fontSize: '14px !important' }} />}
            label={role}
            size="small"
            sx={{ bgcolor: 'rgba(99,102,241,0.12)', color: 'primary.light', fontSize: '0.7rem', justifyContent: 'flex-start' }}
          />
        ))}
        <Chip
          label="Platform Mode"
          size="small"
          sx={{ bgcolor: 'rgba(99,102,241,0.08)', color: 'text.secondary', fontSize: '0.7rem' }}
        />
      </Box>
    </Box>
  )
      ...
      >
        <Toolbar>
          ...
          <IconButton onClick={handleAvatarClick} size="small">
            <Avatar sx={{ width: 34, height: 34, bgcolor: 'primary.main', fontSize: '0.9rem' }}>P</Avatar>
          </IconButton>
          <Menu
            anchorEl={anchorEl}
